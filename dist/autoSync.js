...
export class AutoSyncWatcher {
    ...
    constructor(opts) {
        ...
        this.reconcileIntervalMs = AutoSyncWatcher.adaptivePollInterval(opts.db.getNodeCount(opts.projectName));
    }
    /** Compute reconciliation cadence from node count. Polling is the safety net; watcher events are the hot path. */
    static adaptivePollInterval(nodeCount) {
        if (nodeCount < 2_000)
            return 60_000; // 1 min — reconciliation cadence
        if (nodeCount < 5_000)
    ...
    async pollForChanges() {
        if (this.disposed || this.reindexing)
            return;
        const t0 = Date.now();
        this.pollCount++;
        const { changed, totalHashes } = await this.collectChangedFiles();
        const elapsedMs = Date.now() - t0;
        const shouldLog = changed.length > 0 || this.pollCount % POLL_LOG_EVERY_N === 0 || elapsedMs > POLL_LOG_SLOW_MS;
        if (shouldLog) {
            this.logger.info(`[trace:autoSync.poll] collectChangedFiles in ${elapsedMs}ms hashes=${totalHashes} changed=${changed.length}`);
        }
        if (changed.length > 0) {
     ...
     */
    async checkFileChanged(absolutePath, record) {
        try {
     ...
     */
    handleReindexResult(result, startTime) {
        this.logger.info(`[trace:autoSync.reindex] done in ${Date.now() - startTime}ms success=${result.success} files=${result.filesIndexed} nodes=${result.nodesCreated} errors=${result.errors.length}`);
    ...
    async triggerReindex() {
        if (this.disposed || this.reindexing)
            return;
        this.logger.info(`[trace:autoSync.triggerReindex] pendingEventsSize=${this.pendingEvents.size} reindexing=${this.reindexing} hintPaths=${this.watcherHintPaths.length}`);
     ...
     */
    async initWithLaunchDiff() {
        if (this.disposed)
            return;
        const t0 = Date.now();
        this.logger.info('[trace:autoSync.initWithLaunchDiff] dispatching to worker root=%s', this.opts.projectRoot);
        try {
            const result = await this.opts.workerClient.runLaunchDiff({
                projectRoot: this.opts.projectRoot,
                projectName: this.opts.projectName,
            });
            this.logger.info('[trace:autoSync.initWithLaunchDiff] done elapsed=%dms stale=%d', Date.now() - t0, result.staleCount);
        }
        catch (err) {
            this.opts.onError?.(err instanceof Error ? err : new Error(String(err)));
        }
     ...
     */
    onFileChange(relativePaths) {
        if (this.disposed || relativePaths.length === 0)
            return;
     ...
     */
    onWorkspaceSwitch(newProjectRoot, newProjectName) {
        if (this.disposed)
            return;
        this.stop();
        ...
        this.reconcileIntervalMs = AutoSyncWatcher.adaptivePollInterval(this.opts.db.getNodeCount(newProjectName));
        this.start();
    }
    // ─── Status ───────────────────────────────────────────────────────────────
