 ...
 */
function getLastChangeTimeMs(rootPath) {
    try {
 ...
 */
async function reindexIfStale(rootPath) {
    try {
        ...
        let stale;
        if (!project) {
            console.error(`[trace:graph-mcp.reindex-if-stale] no project record for '${context.projectName}' — treating as stale`);
            stale = true;
        }
        else {
            ...
            if (stale) {
                console.error(`[trace:graph-mcp.reindex-if-stale] stale (last-change=${lastChangeMs} indexed_at=${project.indexed_at})`);
            }
        }
        if (stale) {
        ...
        console.error(`[trace:graph-mcp.reindex-if-stale] error: ${err instanceof Error ? err.message : String(err)}`);
    }
    process.exit(0);
}
...
async function main() {
    ...
    const server = new Server({ name: 'codebase-graph-mcp', version: '0.6.0' }, { capabilities: { tools: {} } });
