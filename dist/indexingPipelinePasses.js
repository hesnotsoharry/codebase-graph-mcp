...
import { emitHeritageEdges } from './indexingPipelineHeritage.js';
import { buildDefProps, buildFileEdges, buildFileNodes, buildFileQnMap, buildFolderEdges, buildFolderNodes, getOrCreatePackageNode, resolveRelativeImport, } from './indexingPipelineSupport.js';
// ─── Structure Pass (Pass 1) ─────────────────────────────────────────────────
export function structurePass(db, projectName, projectRoot, files) {
    db.insertNode({
        id: projectName,
        project: projectName,
        label: 'Project',
...
// ─── Parse Pass (Pass 2) ─────────────────────────────────────────────────────
async function readAndParseOne(parser, file) {
    let content;
    try {
...
export async function parsePass(parser, files, onProgress) {
    let processed = 0;
...
function updateFileProps(db, fileQn, file) {
    const existingFile = db.getNode(fileQn);
    if (!existingFile || !file.parsed)
        return;
...
function collectDefinitions(file, fileQn, acc) {
    if (!file.parsed)
        return;
...
function addRouteNodes(file, fileQn, acc) {
    if (!file.parsed)
        return;
 ...
 */
function collectChunkAccumulator(projectName, files) {
    const acc = { nodes: [], edges: [], projectName };
 ...
 */
function processChunkNodes(db, projectName, files) {
 ...
 */
function processChunkEdges(db, projectName, files) {
    const acc = collectChunkAccumulator(projectName, files);
    db.insertEdges(acc.edges);
}
export function definitionPass(db, projectName, indexedFiles, options) {
...
function processImportChunk(db, projectName, files, fileQnMap) {
    const edges = [];
    for (const file of files) {
        if (!file.parsed)
            continue;
        ...
        for (const imp of file.parsed.imports) {
            const targetQn = imp.source.startsWith('.')
...
export function importPass(db, projectName, indexedFiles, options) {
...
//# sourceMappingURL=indexingPipelinePasses.js.map