...
export function buildFolderNodes(projectName, files) {
...
export function buildFolderEdges(projectName, files) {
    const folders = new Set();
    for (const file of files) {
        const parts = file.relativePath.split('/');
        for (let i = 1; i < parts.length; i++) {
    ...
    for (const folderPath of folders) {
        ...
        const parentQn = parentPath === '.' ? projectName : `${projectName}.${parentPath.replace(/\//g, '.')}`;
...
export function buildFileNodes(projectName, files) {
    return files.map((f) => {
...
export function buildFileEdges(projectName, files) {
    return files.map((f) => {
        const folderPath = path.posix.dirname(f.relativePath);
        const parentQn = folderPath === '.' ? projectName : `${projectName}.${folderPath.replace(/\//g, '.')}`;
...
// ─── Import pass helpers ──────────────────────────────────────────────────────
export function buildFileQnMap(projectName, files) {
    const fileQnMap = new Map();
    for (const file of files) {
...
export function resolveRelativeImport(importSource, fileDir, fileQnMap) {
...
export function getOrCreatePackageNode(db, projectName, importSource) {
...
//# sourceMappingURL=indexingPipelineStructure.js.map