...
export { buildFileEdges, buildFileNodes, buildFileQnMap, buildFolderEdges, buildFolderNodes, getOrCreatePackageNode, resolveRelativeImport, } from './indexingPipelineStructure.js';
 ...
 */
export function isPathSkipped(filePath) {
    if (WORKTREE_RE.test(filePath))
        return true;
    const segments = filePath.replace(/\\/g, '/').split('/');
    return segments.some((seg) => ALWAYS_IGNORE_DIRS.has(seg));
}
// ─── .gitignore / .cbmignore loading ─────────────────────────────────────────
export async function loadIgnoreRules(projectRoot, extraIgnores) {
    const ig = ignore();
    try {
 ...
 */
function isInsideProjectRoot(resolved, projectRoot) {
 ...
 */
async function resolveSymlink(entry, fullPath, projectRoot) {
    let resolved;
    try {
...
async function processDirectory(name, relPath, fullPath, ctx) {
    if (ALWAYS_IGNORE_DIRS.has(name))
        return;
    if (ctx.ig.ignores(relPath + '/'))
        return;
    await walkDirectoryImpl(fullPath, ctx);
}
async function processFile(name, relPath, fullPath, ctx) {
    if (ALWAYS_IGNORE_FILES.has(name))
        return;
    const ext = path.extname(name).slice(1).toLowerCase();
    if (ALWAYS_IGNORE_EXTENSIONS.has(ext))
        return;
    if (ctx.ig.ignores(relPath))
        return;
    try {
        ...
        if (stat.size > ctx.maxSize || stat.size === 0)
            return;
        ctx.files.push({
 ...
 */
async function classifyEntries(entries, dir, projectRoot) {
    const dirs = [];
    const fileEntries = [];
...
async function walkDirectoryImpl(dir, ctx) {
...
export async function walkDirectory(dir, ctx) {
...
export function isEntryPoint(def, file) {
    if (def.name === 'main')
        return true;
    if (def.decorators.some((d) => ENTRY_DECORATORS.has(d)))
        return true;
    if (/\.(test|spec)\.[^.]+$/.test(file.relativePath))
        return true;
    if (def.isDefault && /\/index\.[^.]+$/.test(file.relativePath))
        return true;
    return false;
}
export function buildDefProps(def, file) {
    const props = {
        name: def.name,
...
//# sourceMappingURL=indexingPipelineSupport.js.map