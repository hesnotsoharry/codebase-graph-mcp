 ...
 */
function extractSitesFromLine(lineText, lineNumber) {
    const sites = [];
 ...
 */
function scanSourceForTypeof(source) {
    const sites = [];
...
function buildFileQn(projectName, relativePath) {
    return `${projectName}.${relativePath.replace(/\//g, '.').replace(/\.[^.]+$/, '')}`;
}
function readFileSource(file) {
    try {
...
function resolveTypeofEdges(indexedFiles, projectName, symbolsByName, edges) {
    for (const file of indexedFiles) {
        ...
        for (const site of sites) {
            ...
            if (candidates.length === 0)
                continue; // Skip unresolved (matches callResolutionPass behavior)
 ...
 */
function deduplicateTypeofEdges(edges) {
    const byKey = new Map();
    for (const e of edges) {
        ...
        if (!byKey.has(key)) {
            ...
            merged.edge.props.patterns = [...merged.patterns];
        }
    }
    return [...byKey.values()].map(({ edge }) => edge);
}
// ─── Symbol index builder ─────────────────────────────────────────────────────
function buildTypeofSymbolsByName(db, projectName) {
...
export function typeofResolutionPass(db, projectName, _projectRoot, indexedFiles) {
...
//# sourceMappingURL=indexingPipelineTypeofResolution.js.map