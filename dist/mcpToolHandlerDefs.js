...
export { handleGetArchitecture, handleIndexStatus } from './mcpToolHandlerStructured.js';
...
export async function handleListProjects(ctx) {
    try {
        const projects = ctx.db.listProjects();
        if (projects.length === 0)
            return 'No projects indexed yet.';
        return truncate(projects
            .map((p) => `${p.name}: ${p.node_count} nodes, ${p.edge_count} edges (indexed ${new Date(p.indexed_at).toISOString()})`)
            .join('\n'));
    }
    catch (err) {
...
export async function handleGetGraphSchema(ctx) {
    try {
...
// ─── search_code handler ──────────────────────────────────────────────────────
export async function handleSearchCode(args, ctx) {
    try {
        ...
        if (result.hasMore)
            lines.push(`... more results available. Use offset=${(args.offset ?? 0) + result.results.length}`);
        return truncate(lines.join('\n'));
    }
    catch (err) {
...
function resolveQualifiedName(value, ctx) {
    if (ctx.db.getNode(value))
        return { qn: value };
    ...
    if (exact.length === 1)
        return { qn: exact[0].qualified_name };
    if (exact.length > 1) {
...
function isFileStale(filePath, ctx) {
...
export async function handleGetCodeSnippet(args, ctx) {
    try {
...
function parseTraces(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw.filter((t) => typeof t === 'object' &&
        t !== null &&
        typeof t.fromId === 'string' &&
        typeof t.toId === 'string' &&
        typeof t.type === 'string');
}
export async function handleIngestTraces(args, ctx) {
    try {
        const rawTraces = args.traces;
        if (!rawTraces)
            return "Error: missing required parameter 'traces'";
        if (typeof rawTraces !== 'string')
            return "Error: parameter 'traces' must be a JSON string";
        let parsed;
        try {
        ...
        if (traces.length === 0)
            return 'No valid trace edges in payload.';
...
//# sourceMappingURL=mcpToolHandlerDefs.js.map