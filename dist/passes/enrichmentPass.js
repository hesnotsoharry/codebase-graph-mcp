...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function isEntryPoint(props, filePath) {
    const decorators = props.decorators ?? [];
    if (decorators.some((d) => ENTRY_DECORATORS.has(d)))
        return true;
    if (filePath && /\/index\.[^.]+$/.test(filePath) && props.is_exported)
        return true;
    return false;
}
// ─── Pass implementation ─────────────────────────────────────────────────────
export function enrichmentPass(db, projectName) {
    // IMPLEMENTS and EXTENDS edges are emitted from `definitionPass` (since
    // Wave 21) — see indexingPipelinePasses.ts. enrichmentPass remains
    // focused on entry-point heuristics.
...
//# sourceMappingURL=enrichmentPass.js.map