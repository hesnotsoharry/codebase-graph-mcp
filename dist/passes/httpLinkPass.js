...
const HTTP_CALL_PATTERNS = new Map([
...
function resolveHttpMethods(calleeName, receiverName) {
 ...
 */
function matchUrlToRoute(urlSegments, routeSegments, urlMethod, routeMethod) {
    // Method gate — wildcard route method always passes.
    const routeMethodUpper = routeMethod.toUpperCase();
    if (routeMethodUpper !== '*' && routeMethodUpper !== urlMethod.toUpperCase())
        return null;
    // Segment count gate.
 ...
 */
function scoreHeuristic(callerName, routeMethod, routePath, methods) {
    const routeMethodUpper = routeMethod.toUpperCase();
    const hasWildcardMethod = methods.includes('*');
    if (!hasWildcardMethod && !methods.some((m) => m.toUpperCase() === routeMethodUpper))
        return 0;
    let score = 0.3;
...
function processFileHttpCalls(file, fileQn, projectName, routes) {
    if (!file.parsed)
        return [];
    const edges = [];
    ...
    for (const call of file.parsed.calls) {
        ...
        if (call.firstArgValue !== undefined) {
            ...
            if (bestMatch) {
                const routeProps = bestMatch.route.props;
                edges.push({
                    project: projectName,
                    source_id: callerQn,
                    ...
                    props: {
                        ...
                        resolution_method: bestMatch.result.resolutionMethod,
                    },
                });
            }
        ...
        for (const route of routes) {
            const routeProps = route.props;
            const score = scoreHeuristic(enclosingDef.name, routeProps.method ?? 'GET', routeProps.path ?? '', methods);
            if (score > bestScore) {
                bestScore = score;
                bestRoute = route;
            }
        ...
        if (bestRoute !== null && bestScore > 0) {
            const routeProps = bestRoute.props;
            edges.push({
                project: projectName,
                source_id: callerQn,
...
// ─── Pass implementation ─────────────────────────────────────────────────────
export function httpLinkPass(db, projectName, indexedFiles) {
...
//# sourceMappingURL=httpLinkPass.js.map