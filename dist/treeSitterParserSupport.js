...
export function detectTypeOnlyImport(node) {
...
export function collectNamedImports(node, specifiers) {
...
function collectDefaultImportFromClause(importClause, specifiers) {
...
function collectDefaultImportFallback(node, specifiers) {
    const defaultIdent = node
        .descendantsOfType('identifier')
        .find((n) => n.parent?.type === 'import_clause' || n.parent?.type === node.type);
    if (!defaultIdent)
        return;
    if (specifiers.some((s) => s.name === defaultIdent.text))
        return;
    if (defaultIdent.text === 'type')
        return;
...
export function collectNamespaceImports(node, specifiers) {
...
export function collectDefaultImport(node, specifiers) {
...
function makeAliasedSpecifier(nameNode, aliasNode, isNamespace) {
...
export function extractPythonFromStatement(node) {
    ...
    for (const child of node.namedChildren) {
        ...
        if (child.type === 'dotted_name') {
            ...
            if (nameNode)
                specifiers.push(makeAliasedSpecifier(nameNode, child.lastNamedChild, false));
        }
    }
    return {
        source: importSource,
...
export function extractPythonPlainImport(node) {
    ...
    if (aliases.length > 0) {
        ...
        for (const alias of aliases) {
            ...
            if (nameNode)
                specifiers.push(makeAliasedSpecifier(nameNode, alias.lastNamedChild, true));
        }
    }
    return {
        source: importSource,
...
export { extractCallNodeInfo, extractHandlerName, extractHttpCallArgs, extractRouteCandidate, } from './treeSitterParserCalls.js';
// ─── isNodeExported helpers ───────────────────────────────────────────────────
function isExportedGo(node) {
    const nameNode = node.childForFieldName('name');
    return nameNode ? /^[A-Z]/.test(nameNode.text) : false;
}
function isExportedRust(node) {
    return node.children.some((c) => c.type === 'visibility_modifier');
}
function isExportedJavaLike(node) {
...
export function resolveExportStatus(node, config) {
    if (config.id === 'go')
        return isExportedGo(node);
    if (config.id === 'rust')
        return isExportedRust(node);
    if (config.id === 'java' || config.id === 'c_sharp')
        return isExportedJavaLike(node);
    return true; // Python, Ruby, PHP: no explicit export
}
// ─── extractDecorators helpers ────────────────────────────────────────────────
function collectTsDecorators(node, decorators) {
...
function collectJavaAnnotations(node, decorators) {
    let sibling = node.previousNamedSibling;
...
export function collectDecorators(node) {
    const decorators = [];
...
export function extractDefinitionNameNode(node) {
...
export function isArrowOrFunctionValue(valueNode) {
...
export { buildNodeTypeToLabelMap, collectExportedIdentifiers, extractArrowDeclarator, extractNodeSignature, extractReturnType, extractReturnTypeFromAnnotation, extractSingleDefinition, extractTopLevelNames, findAncestorOfType, hasModifier, isDefaultExport, isNodeExported, } from './treeSitterParserDefs.js';
// ─── Language-specific import extractors (re-exported from companion file) ────
export { dispatchNonTsImport, extractCInclude, extractGoImport, extractJavaLikeImport, extractPhpImport, extractRubyImport, extractRustImport, } from './treeSitterParserImports.js';
//# sourceMappingURL=treeSitterParserSupport.js.map