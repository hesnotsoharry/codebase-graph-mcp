...
- **`codebase_graph` MCP server was NOT attached this session** — graph-backed verification (dead-export scan, blast-radius) ran degraded/deferred; gate confidence rested on build + full Vitest suite (831 green) + per-phase reviews. Re-attach the graph server for graph-backed checks (`verify-wave` can then re-run cleanly).
