...
function isKnownConfigFile(relativePath: string): boolean {
  return CONFIG_FILENAME_PATTERNS.some((re) => re.test(relativePath));
}

export interface ParseAnomalyResult {
  count: number;
  files: string[]; // complete sorted list of anomalous relative paths
}
...
export function countParseAnomalies(indexedFiles: IndexedFile[]): ParseAnomalyResult {
  ...
  for (const file of indexedFiles) {
    ...
    if (isKnownConfigFile(file.relativePath)) continue;
