 ...
 */
function buildReferencesValidNodeIds(db: GraphDatabase, projectName: string): Set<string> {
  const all = db
...
function resolveSymbolDeclaration(
  ...
  try {
    ...
    if (!symbolName || symbolName === '__type' || symbolName === '__function') return null;
...
function collectDecoratorReferences(
  ...
  for (const dec of decorators) {
    ...
    if (!enclosingName) {
      ...
      if (decoratedKind !== ts.SyntaxKind.ClassDeclaration) continue;
      const sourceName = (decorated as import('ts-morph').ClassDeclaration).getName() ?? null;
...
function getDecoratorNameNode(
  ...
  if (kind === ts.SyntaxKind.CallExpression) {
    const callee = (expr as import('ts-morph').CallExpression).getExpression();
    if (callee.getKind() === ts.SyntaxKind.PropertyAccessExpression) {
      return (callee as import('ts-morph').PropertyAccessExpression).getNameNode();
    }
...
function collectJsxReferences(
  ...
  const all: Array<import('ts-morph').JsxSelfClosingElement | import('ts-morph').JsxOpeningElement> =
    [...selfClosing, ...opening];
 ...
 */
function resolveReferencesForFile(
  sourceFile: import('ts-morph').SourceFile,
  projectName: string,
  projectRoot: string,
  fileQn: string,
  ...
  for (const raw of allRawRefs) {
    ...
    if (!validNodeIds.has(sourceQn)) continue;

    const relPath = absoluteToRelative(raw.filePath, projectRoot);
    if (!relPath) continue; // target is outside the project

    const targetFileQn = buildFileQn(projectName, relPath);
    const targetQn = buildSymbolQn(targetFileQn, raw.symbolName);
    if (!validNodeIds.has(targetQn)) continue;
    if (targetQn === sourceQn) continue; // skip self-references
 ...
 * TS/TSX files only.
 */
export async function referencesPass(
  db: GraphDatabase,
  projectName: string,
  projectRoot: string,
  indexedFiles: IndexedFile[],
  options: ReferencesPassOptions,
): Promise<void> {
  ...
  for (const file of tsFiles) {
    ...
    try {
      ...
      log.warn(
        '[trace:refsPass] insertEdges failed file=%s: %s',
        file.relativePath,
        err instanceof Error ? err.message : String(err),
      );
    }
    ...
    log.info(
      '[trace:refsPass] file=%s references=%d',
      file.relativePath,
      edges.length,
    );
  }
